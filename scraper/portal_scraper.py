...
class PortalScraper(Authenticator, CurriculumFetcher, AcademicHistoryFetcher):
    ...
    def __init__(self, cookies: dict[str, str], pensum_version: int = 0):
        ...
        for name, value in cookies.items():
            if name in ("user_name", "udeasecure"):
                self._session.cookies.set(name, value, domain=".udea.edu.co", path="/")
            elif name == "phpsessid_ayudame2":
                self._session.cookies.set("PHPSESSID", value, domain="ayudame2.udea.edu.co", path="/")
    ...
    def _do_reauth(self, host: str, password: str) -> str | None:
        ...
        h = data["hash"]
        self._session.cookies.set("udeasecure", h, domain=".udea.edu.co", path="/")

        end_resp  = self._session.get(f"{reauth_end}?hash={h}", allow_redirects=True)
        end_soup  = BeautifulSoup(end_resp.text, "lxml")
        auto_form = end_soup.find("form")
        if auto_form:
            action = auto_form.get("action", "")
            if action:
                final_resp = self._session.post(action, data={}, allow_redirects=True)
    ...
    def _tsone_get(self, url: str) -> requests.Response:
        if self._udeasecure:
            self._session.cookies.set("udeasecure", self._udeasecure, domain=".udea.edu.co", path="/")
    ...
    def _fetch_semester_grades(self, sem_code: str) -> dict[str, tuple[float, str]]:
        resp = self._tsone_get(self._HISTORIA_URL + f"?app=ver_semestre&semestre={sem_code}")
    ...
    def _fetch_semester_codes(self) -> list[str]:
        resp = self._tsone_get(self._HISTORIA_URL + "?app=consultar")
        soup = BeautifulSoup(resp.text, "lxml")
        codes = [el.get("data-semestre") for el in soup.find_all(attrs={"data-semestre": True})]
        ...
        for h6 in soup.find_all("h6"):
            if "MATERIAS OPCIONALES CON NOTA PENDIENTE" in h6.get_text():
                ...
                for li in ul.find_all("li"):
                    ...
                    if not strong:
                        continue
                    tail = li.get_text(strip=True)[len(strong.get_text(strip=True)):]
    ...
    def fetch_elective_banks(self) -> list[ElectiveBank]:
        ...
        for child in alert_div.children:
            ...
            if child.name == "h6" and "alert-heading" in child.get("class", []):
                current_name     = child.get_text(strip=True).replace("BANCO:", "").strip()
                current_required = 0
                current_approved = 0
    ...
    def fetch_program_info(self, program_code: str) -> tuple[int, list[int]]:
        ...
        for p in resp.json():
            if str(p["codigo"]) == program_code:
                ...
                raw = p.get("versiones", "")
                versiones = [int(v) for v in raw.split(",") if v.strip().isdigit()]
    ...
    def fetch_curriculum(self) -> list[Subject]:
        ...
        for item in cursum_items:
            ...
            subjects.append(Subject(
                ...
                prerequisites=[str(r["materiaRequisito"]) for r in reqs if r["tipoRequisito"] == "PRERREQ"],
                corequisites=[str(r["materiaRequisito"]) for r in reqs if r["tipoRequisito"] == "CORREQ"],
                cursada=is_cursada,
                nota=nota,
